...
def pickle_it(dataSet, file_name):
    with tf.gfile.GFile(file_name, 'wb') as pickle_file:
        print('pickling -> ' + file_name)
        pickle.dump(dataSet, pickle_file)
        print('over')
        
def make_raw_dataset(filepath, targetpath, mirror=True):
#    if not os.path.exists(targetpath):
#        os.system('mkdir '+targetpath)
    file_list = tf.gfile.ListDirectory(filepath)
    print(file_list)
    file_num = 0
    print('begin')
    for filename in file_list:
        ...
        print(tf.gfile.Exists(os.path.join(filepath, filename)))
        matdata = np.transpose(sio.loadmat(
                os.path.join(filepath, filename))['originSet'])
        ...
        dataset = ImgDataSet()
        for ii in range(num_of_data):
            dataset.add_data(matdata[ii], data_type)
        print(dataset.num_examples())
        del matdata
        
        dataset.make(shuffle=True, clean=True)
        file_num += 1
        file_name = os.path.join(targetpath, 'input_'+source_type+'_'+str(file_num)+'.png')
...
#    raw_filepath = "/home/codeplay2017/code/lab/code/paper/realwork/image/time_series_step1_2048/"
#    cwt_targetpath = "../resources/py2/data4cwt_50Hz_256x256_step1/"
#    raw_targetpath = "../resources/py2/data4cwt_50Hz_256x256_step1/"
    
#    make_cwt_dataset(cwt_filepath)
#    make_raw_dataset(raw_filepath, raw_targetpath)
    make_raw_dataset(FLAGS.buckets, FLAGS.checkpointDir)
