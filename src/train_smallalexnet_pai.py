...
def deepnn(x, keep_prob):
    ...
    Args:
        x: an input tensor with the dimensions (N_examples, 240*200)
    Returns:
    ...
    with tf.name_scope('conv1'):
        num_feature1 = 10
        W_conv1 = weight_variable([11, 11, 1, num_feature1])
...
#    tf.summary.image('pool1', h_pool1)
  ...
  # Second convolutional layer -- maps 5 feature maps to 5
    with tf.name_scope('conv2'):
        num_feature2 = 20
        W_conv2 = weight_variable([11, 11, num_feature1, num_feature2])
...
#    tf.summary.image('conv2', h_conv2[:,:,:,np.random.randint(0,num_feature1)])
  ...
  # Second pooling layer.
    ...
    with tf.name_scope('conv3_1'):
        num_feature3_1 = 40
        W_conv3_1 = weight_variable([3, 3, num_feature2, num_feature3_1])
        b_conv3_1 = bias_variable([num_feature3_1])
        h_conv3_1 = tf.nn.relu(conv2d(h_pool2, W_conv3_1) + b_conv3_1)
        
    with tf.name_scope('conv3_2'):
        num_feature3_2 = 40
        W_conv3_2 = weight_variable([3, 3, num_feature3_1, num_feature3_2])
        b_conv3_2 = bias_variable([num_feature3_2])
        h_conv3_2 = tf.nn.relu(conv2d(h_conv3_1, W_conv3_2) + b_conv3_2)
        
    with tf.name_scope('conv3_3'):
        num_feature3_3 = 20
        W_conv3_3 = weight_variable([3, 3, num_feature3_2, num_feature3_3])
        b_conv3_3 = bias_variable([num_feature3_3])
  ...
  # is down to 75x50x64 feature maps -- maps this to 256 features.
    with tf.name_scope('fc1'):
        out_size1 = 4096
        W_fc1 = weight_variable([4 * 4 * num_feature3_3, out_size1])
        b_fc1 = bias_variable([out_size1])
        h_pool3_flat = tf.reshape(h_pool3, [-1, 4*4*num_feature3_3])
        h_fc1 = tf.nn.relu(tf.matmul(h_pool3_flat, W_fc1) + b_fc1)
        h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)
        
    with tf.name_scope('fc2'):
        out_size2 = 4096
        W_fc2 = weight_variable([out_size1, out_size2])
        b_fc2 = bias_variable([out_size2])
        h_fc2 = tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
        h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)

  # Map the 256 features to 3 classes, one for each digit
    with tf.name_scope('fc3'):
        W_fc3 = weight_variable([out_size2, 3])
...
def batch_normalization(inputs, is_training=True, out_size=None, epsilon = 0.001, mode="extractable"):
